  ...
  for (i in 1:nrow(elnet_segments_atac_data)){
    ...
    suppressWarnings(suppressMessages(({
      p <- ggplot(data = data_df, aes(x = atac_vector, y = clinical_vector, color = condition_vector, shape = condition_vector)) +
        geom_smooth(aes(group = 1), method = "lm", se = TRUE, color = "black", linetype = "dashed", size = 1.5, show.legend = FALSE, fill = "#c9c9c9") + 
        ...
        scale_color_manual(values = c("#009e73", "#00aeba", "#e7b800")) +  #  colors for different groups
        guides(
          color = guide_legend(override.aes = list(shape = c(15, 16, 17))),  #  color and shape legends
          shape = guide_legend(override.aes = list(color = c("#009e73", "#00aeba", "#e7b800")))  # Ensure shapes are shown in the legend
        )
    })))
    ...
    suppressWarnings(suppressMessages({
      ...
      ggsave(plot_output_file_path_png, plot = p, width = 10, height = 8, dpi = 300, bg = "white")

      # Save as png with white background
      plot_output_file_path_png <- paste(plot_output_dir, paste(paste(spearman_correlation$estimate, current_gene_id, current_segment, "dotplot", sep = "_"), ".png", sep = ""), sep = "/")
      ggsave(plot_output_file_path_png, plot = p, width = 10, height = 8, dpi = 300, bg = "white")
    }))
    ...
    suppressWarnings(suppressMessages({
      boxplot <- ggplot(data = data_df, 
        ...
        geom_boxplot(outlier.shape = NA, alpha = 0.55, width = 0.8, size=1, color = "#818181") +  # Add boxplot with semi-transparent fill and no outliers
        ...
        scale_color_manual(values = c("#009e73", "#00aeba", "#e7b800")) +  # Specify colors for different groups
        scale_fill_manual(values = c("#009e73", "#00aeba", "#e7b800"))  # Specify fill colors for different groups
    }))
    ...
    suppressWarnings(suppressMessages({
      ...
      ggsave(boxplot_output_file_path_svg, plot = boxplot, width = 5, height = 8, dpi = 300, bg = "white")

      # Save as png with white background
      boxplot_output_file_path_png <- paste(plot_output_dir, paste(paste(spearman_correlation$estimate, current_gene_id, current_segment, "boxplot", sep = "_"), ".png", sep = ""), sep = "/")
      ggsave(boxplot_output_file_path_png, plot = boxplot, width = 5, height = 8, dpi = 300, bg = "white")  
    }))
