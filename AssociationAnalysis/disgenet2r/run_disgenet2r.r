...
if (TRUE) {
  ...
  if (TRUE) {
    cat(bold(cyan("Importing clinical metadata and regression results\n")))
    ...
    cat(cyan("Importing clinical metadata from: "), "\n", clinical_metadata_path, "\n")
    ...
    cat(cyan("Extracted clinical paramters:"), "\n", paste(column_names, collapse = ","), "\n")

    # Import regression results based on current cwd
    regression_data_dir <- "C:/Users/johan/VSCode_projects/bioinf_master/AssociationAnalysis/BasicCorrelation/combined_correlations_cv_pear_04_threshold/"
    regression_results_list <- import_regression_results(regression_data_dir, column_names)
    cat(cyan("Imported regression results from:"), "\n", regression_data_dir, "\n")
  }
